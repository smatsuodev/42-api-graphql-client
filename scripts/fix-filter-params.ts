 ...
 *
 * このスクリプトは filter[*] にマッチするクエリパラメータに対して:
 *   1. スカラー integer を type: array, items: { type: integer } に変換
 *   2. schema 内の style/explode を削除
 *   3. パラメータレベルに style: form, explode: false を設定
 *
 * boolean/string 型の filter パラメータは変換しない。
 *
 * Usage: bun run scripts/fix-filter-params.ts
 */
...
function fixFilterParams(doc: OpenAPIDocument): number {
  let fixedCount = 0
  ...
  for (const [path, pathItem] of Object.entries(doc.paths)) {
    for (const method of HTTP_METHODS) {
      ...
      for (const param of operation.parameters) {
        ...
        // 1. スカラー型 (integer/number) を配列型に変換
        ...
        // 配列型でない場合 (boolean, string 等) はスキップ
...
console.log(`Processing ${OPENAPI_PATH}`)
